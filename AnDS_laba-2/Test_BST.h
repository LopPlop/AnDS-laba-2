...
#define MAX_SIZE 1000000 // Максимально допустимый размер дерева
...
int test_menu()
{
    ...
    cout << "Введите начальное значение размерности дерева" << endl;
    start = get_value();
    ...
    cout << "Введите конечное значение размерности дерева"<< endl;
    end = get_value();
    while (end > MAX_SIZE)
    {
        cout << "Ошибка: введённое значение начальной размерности превышает допустимое максимальное значение размерности дерева.";
...
void test_rand(int n)
{
    ...
    // Обнуляем счётчики операций самого BST-дерева
    tree.setLastOp(0);
    tree.setTotalOp(0);
    ...
    // Вывод результатов:  
    cout << "Размерность дерева после испытаний: " << tree.getSize() << endl; // Вывод размера дерева после теста 
    cout << "Теоретическая оценка(1.39 * log2(n)): " << 1.39 * (log((double)n) / log(2.0)) << endl; // Теоретической оценки трудоёмкости операций BST
    cout << "Среднее количество переборов на одну вставку: " << I / (n / 2) << endl; // Экспериментальной оценки трудоёмкости вставки 
    cout << "Среднее количество переборов на одно удаление: " << D / (n / 2) << endl; // Экспериментальной оценки трудоёмкости удаления  
    cout << "Среднее количество переборов на один поиск: " << S / (n / 2) << endl; // Экспериментальной оценки трудоёмкости поиска 
...
void test_ord(int n, INT_64 delta)
{
    ...
    for (int i = 0; i < n / 2; i++)
    {
        if (i % 10 == 0)  // 10% промахов 
        {
            ...
            catch (int) { S += tree.getLastOp(); } // Обработка исключения при ошибке операции поиска 
        }
        else  // 90% успешных операций 
        {
            int index = rand() % n;
    ...
    // Вывод результатов:
    cout << "Размерность дерева после испытаний: " << tree.getSize() << endl; // Вывод размера дерева после теста 
    cout << "Теоретическая оценка(n / 2): " << n / 2 << endl; // Теоретической оценки трудоёмкости операций BST 
    cout << "Среднее количество переборов на одну вставку: " << I / (n / 2) << endl; // Экспериментальной оценки трудоёмкости вставки
    cout << "Среднее количество переборов на одно удаление: " << D / (n / 2) << endl; // Экспериментальной оценки трудоёмкости удаления
    cout << "Среднее количество переборов на один поиск: " << S / (n / 2) << endl; // Экспериментальной оценки трудоёмкости поиска
    delete[] arr; // Освобождение памяти массива arr[]
